...
(define test (make-test ; (r)
              (legacy equal? ; (run-test legacy)
                      ...
                      [(begin (reset-global-env) (eval-one-exp '(letrec ((f (lambda (n) (if (zero? n) 0
                                                                                            (+ 4 (g (sub1 n))))))
                                                                         (g (lambda (n) (if (zero? n) 0 (+ 3 (f (sub1 n)))))))
                                                                  (g (f (g (f 5))))))) 773 1] ; (run-test legacy 3)
                      [(begin (reset-global-env) (eval-one-exp '(define rotate-linear (letrec
                                                                                          ((reverse (lambda (lyst revlist)
                                                                                                      (if (null? lyst) revlist (reverse (cdr lyst)
                                                                                                                                        (cons (car lyst) revlist)))))) (lambda (los) (let loop ((los los) (sofar '())) (cond ((null? los) los) ((null? (cdr los)) (cons (car los) (reverse sofar '()))) (else (loop (cdr los) (cons (car los) sofar))))))))) (eval-one-exp '(rotate-linear '(1 2 3 4 5 6 7 8)))) '(8 1 2 3 4 5 6 7) 1] ; (run-test legacy 4)
                      [(begin (reset-global-env) (eval-one-exp '(let ((r 2) (ls '(3)) (count 7)) (let loop ()
                                                                                                   (if (> count 0) (begin (set! ls (cons r ls))
                                                                                                                          (set! r (+ r count))
                                                                                                                          (set! count (- count 1)) (loop))))
                                                                  (list r ls count)))) '(30 (29 27 24 20 15 9 2 3) 0) 1] ; (run-test legacy 5)
