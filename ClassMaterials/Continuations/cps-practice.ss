...
(define intersection-cps
  (lambda (los1 los2 k)
    (if (null? los1) 
        (apply-k k '())
        (memq-cps (car los1) los2 (make-k
                                   (lambda (ls/f) ;we don't know if the result is true or a list
                                     (if ls/f
                                         (intersection-cps (cdr los1) los2
                                                           (make-k (lambda (intersection-cdr)
                                                                     (apply-k k (cons (car los1) intersection-cdr)))) )
                                         (intersection-cps (cdr los1) los2)
                                         )))))))
