...
(define (for-exp? exp)
  (and (list? exp) (<= 7 (length exp)) (equal? 'for (1st exp)) (equal? 'from (3rd exp)) (equal? 'to (fifth exp)) ))
    ...
    (cond
      ...
      [(pair? datum)
       (cond
         ;;for-exp
         [(for-exp? datum)
          (for-exp (2nd datum) (parse-exp (fourth datum))  (parse-exp (sixth datum)) (map parse-exp (drop datum 6)) )
          ]
         ;OR STATEMENT
         [(equal? 'or (1st datum))
...
(define (expand-cond exps bodies)
  (let loop ([exps exps] [bodies bodies])
    ...
    (cond
      ...
      ;(if-else-exp (syntax-expand (car exps)) (syntax-expand (car bodies)) (syntax-expand (car bodies)))
      ;      (if-exp (syntax-expand (car exps)) (syntax-expand (car bodies))))]
      [else (displayln "else")
            (displayln (syntax-expand (car exps)))
            (displayln (syntax-expand (car bodies)))
            (if-else-exp (syntax-expand (car exps)) (syntax-expand (car bodies)) (loop (cdr exps) (cdr bodies)))])))

;      [cond-exp (conds bodies) (if (null? conds)
;                                   'void
;                                   (if (eqv? (car conds) 'else)
;                                       (car bodies)
;                                       (eval-exp (parse-exp (list 'if (car conds)
;                                                                  (car bodies)
;                                                                  (syntax-expand (cond-exp (cdr conds) (cdr bodies)) env) )) env)))]
...
(define *prim-proc-names* '(+ - * add1 sub1 cons = / not zero? >= car cdr list null? eq? equal?
                              length list->vector list? pair? vector->list
                              vector? number? symbol? caar cadr cadar procedure? vector vector-ref vector-set!
                              map apply < > quotient negative? positive? set-car!))
