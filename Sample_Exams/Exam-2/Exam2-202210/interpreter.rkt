    ...
    (cond
      ...
      [(pair? datum)
       (cond
         ...
         ;Simplecase
         [(equal? 'simplecase (1st datum))
          (simplecase-exp (parse-exp (2nd datum))
                          (map car (cddr datum))
                          (map parse-exp (map 2nd (cddr datum))))
          ]
         ;BEGIN EXPRESSION
...
(define syntax-expand
  (lambda (exp)
    ...
    (cases expression exp
      ...
      ;simplecase
      [simplecase-exp (test-exp syms bodies)
                      (expand-simplecase test-exp syms bodies)
                      ]
      [else
       exp]
      )))
...
[app-exp
 (rator expression?)
 (rand (list-of? expression?))] [a, b, c] [1, 2, 3]
|#
(define (expand-simplecase test-exp syms bodies)
  (cond
   
    [(null? (cdr syms)) (if-exp (app-exp (var-exp 'eqv?) (list (syntax-expand test-exp) (quoted-exp (list 'quote (car syms)))))
                                (syntax-expand (car bodies)))
                                     
                        ]
    [else
     (if-else-exp (app-exp (var-exp 'eqv?) (list (syntax-expand test-exp) (quoted-exp (list 'quote (car syms)))))
                  (syntax-expand (car bodies))
                  (expand-simplecase test-exp (cdr syms) (cdr bodies)))
     ;(expand-simplecase test-exp (cdr syms) (cdr bodies))
     ]
    )
  )
