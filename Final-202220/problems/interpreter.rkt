...
;;CHECK FOR NAMED LET
(define (name-let-exp? exp)
  (and (<= 4 (length exp)) (list? exp) (symbol? (2nd exp)) (list? (3rd exp)))
 )
    ...
    (cond
      ...
      [(pair? datum)
       (cond
         ...
         [(equal? 'let (1st datum))
          (cond
            [(name-let-exp? datum) ;this is a named let 
         ...
         ;MAKE-POINT
         [(equal? 'make-point (1st datum))
          (make-point-exp (2nd datum) (3rd datum))
          ]
         ;BEGIN EXPRESSION
